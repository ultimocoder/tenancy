        ...
        <div class="rightside">
            ...
            @include('landlord_layouts.topbar')
                ...
                @if(session()->has('error'))    
                    ...
                    <div>
                        <div class="heading-2">Current plan</div>
                    ...
                    <div class="container mx-0 account-plans">
                        <div class="row mb-3">
                            ...
                            <div class="col-sm-3">
                                <div class="account-plan @if($item->id == $package->package_id) active @endif">
                                    <input type="radio" name="plan" class="package" id="plan{{$item->id}}" data-id="{{$item->id}}" data-uid="{{$item->min_qty}}" @if($item->id == $package->package_id) checked @endif>
                                    <label for="plan{{$item->id}}">
                                        <div>
                                        @if($item->id == $package->package_id)
                                            <div class="current fw-bold fs-18">Current plan</div>
                                        @else
                                        ...
                                        <div class="no-unit">
                                            ...
                                            <input type="number" readonly class="currentUnit" value="@if($package->package_id == $item->id ){{$subscription->quantity}}@else{{$item->min_qty}}@endif" min="{{$item->min_qty}}" max="{{$item->max_qty}}">
...
<script>
    $(function(){
        ...
        $('#next').on('click', function(){
            ...
            var url = '/tenancy/public/landlord/account/subscription/subscription-billing-cycle/'+unit+'/'+pid;
            location.href = url;
        });
